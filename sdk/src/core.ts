// On-chain core: instruction builders and the authorization format for the
// CURVED x402 settlement program. Mirrors client/src/lib.rs (Rust reference).
import {
  Connection,
  Keypair,
  PublicKey,
...
/// v2 session-key split: `delegate` (the agent key) may sign authorizations
/// but can never withdraw; only the opening authority can. Pass the payer
/// itself for a self-owned agent.
export function openEscrowIx(payer: PublicKey, delegate: PublicKey): TransactionInstruction {
  return new TransactionInstruction({
    ...
    data: Buffer.from(cat(disc("open_escrow"), delegate.toBytes())),
  });
}

/// Rotate or revoke (PublicKey.default) the agent key. Authority signs;
/// revocation stops every outstanding signed authorization instantly.
export function setDelegateIx(authority: PublicKey, newDelegate: PublicKey): TransactionInstruction {
  return new TransactionInstruction({
    programId: PROGRAM_ID,
    keys: [
      { pubkey: authority, isSigner: true, isWritable: false },
      { pubkey: escrowPda(authority), isSigner: false, isWritable: true },
    ],
    data: Buffer.from(cat(disc("set_delegate"), newDelegate.toBytes())),
  });
}
...
/// The settle instruction. Permissionless: any `relayer` (including the payee).
...
/// the escrow's delegate, which is only the payer itself for a self-owned agent.
export async function escrowDelegate(conn: Connection, payer: PublicKey): Promise<PublicKey | null> {
  const info = await conn.getAccountInfo(escrowPda(payer));
  if (!info) return null;
  const delegate = new PublicKey(info.data.subarray(40, 72)); // discriminator, authority, then delegate
  return delegate.equals(PublicKey.default) ? null : delegate;
}

/// Relay one signed authorization: [ ed25519_verify, pay ] in one transaction.
/// `signer` is whoever signed the authorization. Leave it out and the escrow is
/// asked, which is what a relayer holding only the payload has to do anyway.
export async function relay(conn: Connection, relayer: Keypair, a: Authorization, sig: Uint8Array, signer?: PublicKey): Promise<string> {
  const authSigner = signer ?? (await escrowDelegate(conn, a.payer));
  if (!authSigner) throw new Error("no escrow for this payer, or its delegate is revoked");
  const tx = new Transaction().add(ed25519Ix(authSigner.toBytes(), sig, authorizationBytes(a)), payIx(relayer.publicKey, a));
